...
012 ⏳ 定稿待发布（皮肤·诊断空白系列二）——下一篇发这个
013 ⏳ 定稿待发布（口腔·诊断空白系列三）——视012反馈决定
014 🔴 暂停（糖尿病前期·诊断空白系列四）——论点框架被深度推演颠覆,等推演A完成后重写
015 🔴 暂停（系列收口）——重新定位为"系列真正高潮",写"三方需求错位"主题
004B ⏳ 骨架     006B ⏳ 骨架
```
...
├── 北极星文档.md                ← 战略锚点
│
├── core/                        ← 稳定层：规则/模板/判断框架（12个文件）
├── theory/                      ← 稳定层：理论输入（25个文件）
...
### ops/（4个文件 + _temp-people/）
```
执行状态总表.md                    ★ 全局进度（含维护触发器，每次开对话先看）
清单草稿-未验证教训.md           ★★★ 新增。零摩擦捕获对话中冒出的新见解（项目审问清单的燃料源）
清单候选-待升级教训.md         ★★★ 新增。三次升候选后结构化整理，等待实战验证升正式清单
